...
def _create_purchase_invoice(
    ...
    *,
    ...
    client.post(
        ...
        headers={"x-actor-role": "inventory_admin"},
        json={"note": "Ready for supplier settlement follow-up"},
    )
    client.post(
        f"/v1/tenants/{tenant_id}/branches/{branch_id}/purchase-orders/{purchase_order['id']}/approve",
        headers={"x-actor-role": "tenant_owner"},
        json={"note": "Approved for supplier settlement follow-up"},
    )
...
def test_supplier_settlement_report_spots_follow_up_suppliers():
    ...
    state.purchase_invoices[paper_invoice["purchase_invoice_id"]]["due_date"] = "2026-04-10"
    state.purchase_invoices[ink_invoice["purchase_invoice_id"]]["due_date"] = "2026-04-15"
