...
def test_goods_receipt_requires_an_approved_purchase_order():
    ...
    assert blocked_receipt.status_code == 400
    assert blocked_receipt.json()["detail"] == "Purchase order must be approved before receiving"

    client.post(
        f"/v1/tenants/{tenant_id}/branches/{branch_id}/purchase-orders/{purchase_order['id']}/submit-approval",
        headers={"x-actor-role": "inventory_admin"},
        json={"note": "Ready for dispatch"},
    )
    client.post(
    ...
    assert approved_receipt.json()["purchase_order_id"] == purchase_order["id"]
