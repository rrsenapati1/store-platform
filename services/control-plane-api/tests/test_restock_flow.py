...
def test_owner_runs_branch_restock_task_workflow_without_posting_inventory_ledger_entries():
    database_url = sqlite_test_database_url("restock-flow")
    client = TestClient(
        create_app(
    ...
    task_id = restock_task.json()["id"]
    assert restock_task.json()["status"] == "OPEN"
    assert restock_task.json()["task_number"] == "RST-BLRFLAGSHIP-0001"
    assert restock_task.json()["suggested_quantity_snapshot"] == 16.0
    ...
    duplicate_task = client.post(
        ...
        json={
            ...
            "note": "Second task should fail",
        },
    )
    assert duplicate_task.status_code == 400
    assert duplicate_task.json()["detail"] == "Active restock task already exists for product"

    picked_task = client.post(
        f"/v1/tenants/{tenant_id}/branches/{branch_id}/restock-tasks/{task_id}/pick",
        headers=owner_headers,
        json={"picked_quantity": 10, "note": "Picked from backroom cage"},
    )
    assert picked_task.status_code == 200
    assert picked_task.json()["status"] == "PICKED"
    assert picked_task.json()["picked_quantity"] == 10.0

    completed_task = client.post(
        f"/v1/tenants/{tenant_id}/branches/{branch_id}/restock-tasks/{task_id}/complete",
        headers=owner_headers,
        json={"completion_note": "Shelf refill done"},
    )
    assert completed_task.status_code == 200
    assert completed_task.json()["status"] == "COMPLETED"
    assert completed_task.json()["completion_note"] == "Shelf refill done"

    restock_board = client.get(
        f"/v1/tenants/{tenant_id}/branches/{branch_id}/restock-board",
        headers=owner_headers,
    )
    assert restock_board.status_code == 200
    assert restock_board.json()["open_count"] == 0
    assert restock_board.json()["picked_count"] == 0
    assert restock_board.json()["completed_count"] == 1
    assert restock_board.json()["records"][0]["status"] == "COMPLETED"
