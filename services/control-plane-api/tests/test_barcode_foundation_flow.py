...
def test_runtime_scan_lookup_returns_null_policy_when_branch_policy_is_missing():
    database_url = sqlite_test_database_url("barcode-foundation-null-policy")
    client = TestClient(
        create_app(
            database_url=database_url,
            bootstrap_database=True,
...
def test_runtime_scan_lookup_returns_automatic_discount_posture_for_targeted_items():
    database_url = sqlite_test_database_url("barcode-foundation-automatic-posture")
    client = TestClient(
        create_app(
            database_url=database_url,
            bootstrap_database=True,
    ...
    tenant = client.post(
        "/v1/platform/tenants",
        headers=admin_headers,
        json={"name": "Acme Retail", "slug": "acme-retail-automatic"},
    )
    assert tenant.status_code == 200
    tenant_id = tenant.json()["id"]
    ...
    assert scan.status_code == 200
    assert scan.json()["automatic_discount_hint"] == {
        "campaign_name": "Tea automatic discount",
        "discount_type": "PERCENTAGE",
        "discount_value": 10.0,
        "scope": "ITEM_CATEGORY",
    }
