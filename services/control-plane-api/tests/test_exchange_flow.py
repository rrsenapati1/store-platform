...
def test_cashier_creates_exchange_and_collects_balance() -> None:
    database_url = sqlite_test_database_url("exchange-flow")
    client = TestClient(
        create_app(
    ...
    tenant = client.post(
        ...
        json={"name": "Acme Retail", "slug": "acme-retail-exchanges"},
    )
    ...
    assert exchange.json()["balance_direction"] == "COLLECT_FROM_CUSTOMER"
    assert exchange.json()["balance_amount"] == 97.13
    assert exchange.json()["status"] == "COMPLETED"
    assert exchange.json()["sale_return"]["status"] == "EXCHANGE_SETTLED"
    assert exchange.json()["sale_return"]["credit_note"]["credit_note_number"] == "SCN-BLRFLAGSHIP-0001"
    assert exchange.json()["replacement_sale"]["invoice_number"] == "SINV-BLRFLAGSHIP-0002"
    assert exchange.json()["payment_allocations"] == [
    ...
    assert inventory_snapshot.json()["records"][0]["stock_on_hand"] == 19.0
