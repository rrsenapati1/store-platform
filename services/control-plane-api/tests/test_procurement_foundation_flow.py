...
def test_owner_bootstraps_supplier_and_purchase_order_foundation():
    database_url = sqlite_test_database_url("procurement-foundation")
    client = TestClient(
        create_app(
    ...
    suppliers = client.get(f"/v1/tenants/{tenant_id}/suppliers", headers=owner_headers)
    assert suppliers.status_code == 200
    assert suppliers.json()["records"][0]["name"] == "Acme Tea Traders"
    ...
    assert purchase_orders.json()["records"][0]["supplier_name"] == "Acme Tea Traders"
    ...
    assert report_before.json()["not_requested_count"] == 1
    ...
    assert report_after.json()["approved_count"] == 1
    assert report_after.json()["records"][0]["purchase_order_id"] == purchase_order_id
    assert report_after.json()["records"][0]["ordered_quantity"] == 24.0
