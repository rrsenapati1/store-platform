...
def test_customer_directory_history_and_branch_report() -> None:
    database_url = sqlite_test_database_url("customer-reporting")
    client = TestClient(
        create_app(
    ...
    tenant = client.post(
        ...
        json={"name": "Acme Retail", "slug": "acme-retail-customers"},
    )
    ...
    sale_total = sale.json()["grand_total"]
    ...
    assert anonymous_sale.status_code == 200
    anonymous_total = anonymous_sale.json()["grand_total"]

    exchange = client.post(
        f"/v1/tenants/{tenant_id}/branches/{branch_id}/sales/{sale.json()['id']}/exchanges",
        headers=cashier_headers,
        json={
            "settlement_method": "UPI",
            "return_lines": [{"product_id": product_id, "quantity": 1}],
            "replacement_lines": [{"product_id": product_id, "quantity": 2}],
        },
    )
    assert exchange.status_code == 200
    replacement_total = exchange.json()["replacement_sale"]["grand_total"]
    credit_note_total = exchange.json()["sale_return"]["credit_note"]["grand_total"]
    ...
    records = directory.json()["records"]
    assert len(records) == 1
    customer_record = records[0]
    assert customer_record["gstin"] == "29AAEPM0111C1Z3"
    assert customer_record["visit_count"] == 2
    assert customer_record["lifetime_value"] == round(sale_total + replacement_total, 2)

    history = client.get(
        f"/v1/tenants/{tenant_id}/customers/{customer_record['customer_id']}/history",
        headers=owner_headers,
    )
    assert history.status_code == 200
    assert history.json()["sales_summary"]["sales_count"] == 2
    assert history.json()["sales_summary"]["return_count"] == 1
    assert history.json()["sales_summary"]["exchange_count"] == 1
    assert history.json()["sales_summary"]["sales_total"] == round(sale_total + replacement_total, 2)
    ...
    assert branch_report.json()["customer_count"] == 1
    assert branch_report.json()["repeat_customer_count"] == 1
    assert branch_report.json()["anonymous_sales_count"] == 1
    assert branch_report.json()["anonymous_sales_total"] == anonymous_total
    assert branch_report.json()["top_customers"][0]["sales_count"] == 2
    assert branch_report.json()["top_customers"][0]["sales_total"] == round(sale_total + replacement_total, 2)
    assert branch_report.json()["return_activity"][0]["return_count"] == 1
    assert branch_report.json()["return_activity"][0]["exchange_count"] == 1
    assert branch_report.json()["return_activity"][0]["credit_note_total"] == round(credit_note_total, 2)


def test_customer_reporting_prefers_customer_profile_identity_for_linked_sales() -> None:
    database_url = sqlite_test_database_url("customer-reporting-profile-identity")
    client = TestClient(
        create_app(
            database_url=database_url,
            bootstrap_database=True,
    ...
    tenant = client.post(
        "/v1/platform/tenants",
        headers=admin_headers,
        json={"name": "Acme Retail", "slug": "acme-retail-customer-profile-report"},
    )
    assert tenant.status_code == 200
    tenant_id = tenant.json()["id"]
    ...
    assert directory.status_code == 200
    assert len(directory.json()["records"]) == 1
    customer_record = directory.json()["records"][0]
    assert customer_record["customer_id"] == customer_profile_id

    history = client.get(
        f"/v1/tenants/{tenant_id}/customers/{customer_profile_id}/history",
        headers=owner_headers,
    )
    assert history.status_code == 200
    assert history.json()["customer"]["customer_id"] == customer_profile_id
