...
def test_owner_queues_barcode_label_print_job_on_control_plane_runtime():
    database_url = sqlite_test_database_url("barcode-print-runtime")
    client = TestClient(
        create_app(
    ...
    print_job = client.post(
        ...
        headers=owner_headers,
        json={"device_id": device_id, "copies": 2},
    )

    assert print_job.status_code == 200
    assert print_job.json()["job_type"] == "BARCODE_LABEL"
    assert print_job.json()["reference_type"] == "catalog_product"
    assert print_job.json()["payload"]["labels"] == [
        {
            "sku_code": "tea-classic-250g",
    ...
    assert queued_jobs.json()["records"][0]["id"] == print_job.json()["id"]
    assert queued_jobs.json()["records"][0]["payload"]["labels"][0]["product_name"] == "Classic Tea"
