...
type OwnerSyncRuntimeSectionProps = {
  accessToken: string;
  tenantId: string;
  branchId: string;
};

export function OwnerSyncRuntimeSection({ accessToken, tenantId, branchId }: OwnerSyncRuntimeSectionProps) {
  ...
  async function loadSyncMonitoring() {
    ...
    try {
      const [statusResponse, conflictResponse, envelopeResponse] = await Promise.all([
        ownerControlPlaneClient.getRuntimeSyncStatus(accessToken, tenantId, branchId),
        ownerControlPlaneClient.listRuntimeSyncConflicts(accessToken, tenantId, branchId),
        ownerControlPlaneClient.listRuntimeSyncEnvelopes(accessToken, tenantId, branchId),
      ]);
      startTransition(() => {
